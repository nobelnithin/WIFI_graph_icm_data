...
#         self.timer = QTimer()
#         self.timer.setInterval(500)  # Interval in milliseconds (adjust as needed)
...
class RealTimeGraph(QMainWindow):
    ...
    def update_plot(self):
        try:
            ...
            # Convert response from hex to int
            hex_value = response.text.strip()
            value = int(hex_value, 16)  # Convert from hexadecimal to integer
            
            # Handle negative values for int16_t
            if value > 0x7FFF:
